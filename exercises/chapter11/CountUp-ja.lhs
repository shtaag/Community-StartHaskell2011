...
プレーヤーは、ある決まった数字の集合（大体現在の年月日）と演算子を使って、目標の値になる
関数を作る。各関数は全ての数字をちょうど一回ずつ使う必要がある。目標の値は0から始まり、
正当な関数を見つかったら増加させていく。

例：数字が`20111116`で、目標の値が`121`だったら、`(20-(6+1+1+1))^(1+1)`が正当な
関数だ。

この練習問題では、Haskellでこのゲームを開発する。

次のモジュール宣言とインポートを無視してよい。
...
ライブラリはこのファイルの下に付いてる。）`show`の実装では明示的に括弧を使うが、この
パーサーは括弧がない関数を取り込める必要がある。以下の結合性と束縛の優先順位を使え。
...
（`1`と`2`）を使っていることに注意せよ。戻り値は昇順のリストとする。`Data.List`からインポート
...
戻り値の型は`Maybe Int`だ。正当な関数だったら、`Just`を使って値を返せ。
以下の場合は`Nothing`を返せ。
...
こういう`Maybe`の使い方は本には書いてない。最初にdo記法を使わずに`eval`を普通に
定義することをお勧めする。その定義が全てのテストを通ったらdo記法を
使ってみると良い。
...
`IO`モナドに入っているので、画面に文字列を表示するために`putStrLn`関数を使ったり、
プレーヤーが入力した文字列を受け取るために`getLine`関数を使ったりすることができる。
...
プレーヤーが同じ行に入力できるように、`putStr`でプロンプトを表示したあと
...
この実装の問題は、`getLine`では入力時を編集することができない点だ。例えば、
括弧を忘れても戻って入力しなおすことができない。これはとても面倒だ。

入力する時に編集できるように、私がhaskelineライブラリを使って他の
`play`関数を定義した。このファイルの全てのテストを通ったら、`main`関数で
ゲームを遊ぶことができる。
