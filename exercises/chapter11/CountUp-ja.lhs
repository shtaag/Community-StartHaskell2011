...
このファイルは文芸的プログラミングの形式で書いている。このファイルを自分の
パソコンにダウンロードして、`CountUp.lhs`に名付けよ。ファイルにある`undefined`
関数を消して、自分の定義で実装せよ。含まれているテストで自分の定義を確認せよ。
全てのテストを受かったらゲームをコンパイルして遊べることができるようになる。
...
「数え上げ」ゲームは、昔、Mixiの数学コミュニティーで遊んだゲームだ。
プレーヤーは、一定の数字（大体現在の年月日）と演算子を使って、目標の値になる
関数を作る。各関数は全ての数字を一回ずつ使う必要。目標の値は0から始まって、
正当な関数を見つかると目標の値を増加させる。

例：数字が`20111116`と目標の値が`121`だたら、`(20-(6+1+1+1))^(1+1)`が正当な
関数だ。

この練習問題にHaskellでこのゲームを開発する。

次のモジュール宣言とインポートを無視する。
...
テストのために`Eq`のインスタンスを自動導出する。二つの関数が全く同じ構文木が
あったら同等だ。例えば、`(1 + 2) + 3`と`1 + (2 + 3)`の構文木が異なるので
同等ではないだ。
...
ライブラリはこのファイルの下に付いてる。）`show`の実装に沢山括弧を使うけどこの
パーサーは括弧がない関数を取り込む必要。以下の結合性と束縛の優先順位を使えよ。
...
関数に使っている数字を戻る関数`digits`を定義せよ。`12`という値は二つの数字
（`1`と`2`）を使う。戻り値はソート純のリストにする。`Data.List`からインポート
された`sort :: Ord a => [a] -> [a]`関数を使うと良い。
...
戻り値の型は`Maybe Int`だ。正当な関数だったら、`Just`を使って値を戻せよ。
以下の場合に`Nothing`を戻せよ。
...
こういう`Maybe`の使い方は本に書いてない。最初にdo記法を使えず`eval`を普通に
定義することをお勧めする。この定義が全てのテストを合格したらdo記法を
使ってみても良い。
...
引数は`String`の型で数字のリストだ。二番目の引数は現在の目標の値だ。この関数は
`IO`モナドに入っているが、画面に文字列を表示するために`putStrLn`関数を使って、
プレーヤーが入力した文字列を受け取るために`getLine`関数を使う。

プレーヤーが数字と現在の目標の値が分かるために、この関数が最初にプロンプトを
表示する。例えば、数字が`20111116`と現在の目標の値が`0`だったら、次のような
プロンプトを表示する。

    {20111116} 0:

プレーヤーが同じ行に入力できるため、`putStr`でプロンプトを表示してから
`hFlush stdout`を実行する。それから`getLine`を使って、入力した文字列を
受け取る。

プレーヤーが「quit」を入力したら止まる。他の値だったら、パーサーで関数に変換
...
この実装の問題は`getLine`が入力する時に編集することができない。例えば、忘れた
括弧を入れることができなくて面倒くさいだ。

入力する時に編集することができるために、私がhaskelineライブラリを使って他の
`play`関数を定義した。このファイルの全てのテストを合格したら、`main`関数で
ゲームを遊ぶことができる。
