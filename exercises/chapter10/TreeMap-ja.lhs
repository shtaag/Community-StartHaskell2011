...
この宣言では、リストと同じように、`a`型変数によってどの型でも持つことができる。
...
もし`Int`に特化した`Tree`の`Show`インスタンス宣言を書いたら、次のように
なる。
...
高階関数`map :: (a -> b) -> [a] -> [b]`は 、リストの各要素に関数を適用して、新しいリストを返す。
`Tree`型に対して同じことをする高階関数`treeMap`を定義せよ。
...
コンテナーの要素に関数を適用することは基本な作業であるので、Haskellにはそのための
`Functor`クラスがある。「Functor」（関手）は圏論の言葉だ。二つの圏の間の対応として
考える。（例えば、`map show [1..10]`は整数のリストの圏から文字列のリストの圏への
対応として考えることができる。）
...
`Functor`の宣言では、型変数`f`はコンテナーの型を表している。例えば、`Tree`の`fmap`は
`fmap :: (a -> b) -> Tree a -> Tree b`という型で、最初の引数で渡す関数を
使って`a`型の`Tree`を`b`型の`Tree`へ写す。
...
（`Show`のインスタンス宣言と異なり、型の制約は必要ない。なので
インスタンス宣言の最初の行に`(Tree a)`と書かない。）
...
`Functor`クラスを使えば、標準の`fmap`関数でコンテナーの要素に関数を
適用することができる。リストも`Functor`のインスタンスなので、リストにも`fmap`を
使うことができる。`map`関数は、コンテナーがリストの場合の特殊な`fmap`として考えられる。

目的
----

この練習問題の目的は、型クラスの使い方の例をもう一つ見せることだ。Haskellを勉強する時、
よくモナドについて語られる。なぜなら、モナドは理解するのが難しいからだ。しかし私の
経験によると、モナドをまだ理解していない人は大体、型変数を持つ型クラスも理解していない。
型変数を持つ型クラスを使えば、抽象化する方法はいくらでもある。モナドはそのうちの一つにすぎない。
モナドを理解するためには、型クラスを理解することが必須だ。（下の補助コード以外、この練習
問題はモナドを使ってない。）
