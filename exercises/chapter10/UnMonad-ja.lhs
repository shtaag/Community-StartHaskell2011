...
do記法を使わず、上のパーサー関数を再定義せよ。`>>=`と`>>`の関数を直接使う
必要がある。`+++`演算子を使って良い。各変数のスコープを明らかにするために
括弧を沢山使って良い。
...
do記法と`+++`演算子を使わず、またパーサー関数を再定義せよ。今回は、`Parser`の
コンストラクター（`P`）を直接使う必要がある。
...
モナド関数（`>>=`、`>>=`、`return`）を使わず、またパーサー関数を再定義せよ。
